...
let shipmentsData = fs.readFileSync(`${__dirname}/../data/shipments.json`);
let shipments = JSON.parse(shipmentsData);
...
exports.createShipment = () =>
  catchAsync(async (req, res, next) => {
    ...
    if (
      ...
      return next(
        new AppError(
          `Please provide all the required fields(name, quantity, ship to address and shipping date)`,
          400
        )
      );
    }
    // Check if the quantity is a number
    if (isNaN(req.query.shipQty)) {
      return next(new AppError(`Please provide a valid quantity`, 400));
    }
    const product = await products.find(
    ...
    fs.writeFile(
      ...
      (err) => {
        if (err) {
          product.quantity += newShipment.shipQty;
          return next(new AppError(`Failed to create the Shipment`, 500));
        }
      }
    );
    fs.writeFile(
